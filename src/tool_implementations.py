...
# in SOLARIS_MCP_ALLOWED_COMMANDS -- a stdio server that genuinely needs an
...
def _mcp_allowed_commands() -> set:
    ...
    path. Empty by default; set SOLARIS_MCP_ALLOWED_COMMANDS (comma-separated)
    to opt specific trusted binaries in. Denied commands are rejected even if
    listed here."""
    raw = os.environ.get("SOLARIS_MCP_ALLOWED_COMMANDS", "")
...
def _validate_mcp_command(command, args, env) -> Optional[str]:
    ...
    if base not in _mcp_allowed_commands():
        return (
            ...
            "SOLARIS_MCP_ALLOWED_COMMANDS if you trust it, or register the "
...
# In-process loopback base for agent tools that call Solaris's own API
...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    """Generic loopback to allowed internal Solaris API endpoints. Lets the
    ...
    for t in merged:
        ...
        if t.get("status") == "error" and t.get("output_tail"):
            ...
            if tail:
                ...
                # banner ("Locale: C / Ubuntu_Solaris ❯") — useless for
...
async def do_tail_serve_output(content: str, owner: Optional[str] = None) -> Dict:
    ...
    # Prefer the persisted /tmp/solaris-tmux/SESSION.log file over the
    ...
    log_path = f"/tmp/solaris-tmux/{session_id}.log"
