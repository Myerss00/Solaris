---
name: solaris
description: Use when the user asks Codex to read or write Solaris data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Codex Agent API. Requires SOLARIS_URL and SOLARIS_API_TOKEN.
---

# Solaris

Use this skill when a user asks to interact with Solaris from Codex.
...
- `SOLARIS_API_TOKEN`: Scoped API token created in Solaris Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in Solaris Settings and expose both values to the terminal session.
...
- All Solaris data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
```bash
python3 integrations/codex/scripts/solaris_api.py capabilities
python3 integrations/codex/scripts/solaris_api.py todos list
python3 integrations/codex/scripts/solaris_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/solaris_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 integrations/codex/scripts/solaris_api.py emails list 5
python3 integrations/codex/scripts/solaris_api.py emails read UID
```
...
```bash
python3 integrations/codex/scripts/solaris_api.py GET /api/codex/memory
python3 integrations/codex/scripts/solaris_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
- Prefer `POST /api/codex/emails/draft-document` for Codex-written email replies. It creates an editable Solaris Document with `language: "email"` and does not touch IMAP/send.
...
```bash
python3 ~/plugins/solaris/scripts/solaris_api.py cookbook tasks
python3 ~/plugins/solaris/scripts/solaris_api.py cookbook output serve-abc12345 400
python3 ~/plugins/solaris/scripts/solaris_api.py cookbook stop serve-abc12345
python3 ~/plugins/solaris/scripts/solaris_api.py cookbook serve \
...
If you are about to reach the Solaris host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Solaris Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
