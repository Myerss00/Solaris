...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('solaris:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('.adm-quickstart-section').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'solaris.addModels.' + sec.id + '.open';
    let open = false;
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { window.dispatchEvent(new CustomEvent('solaris-integrations-changed')); } catch (_) {}
      });
    });
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'solaris_backup.json';
