...
let _sortMode = Storage.get('solaris-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Solaris Chat';
...
const FOLDER_STATE_KEY = 'solaris-folder-state';
const FOLDER_ORDER_KEY = 'solaris-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Solaris kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Solaris Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('solaris-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('solaris-session-sort', mode);
  else Storage.remove('solaris-session-sort');
  renderSessionList();
}
