...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'solaris-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('solaris-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('solaris-compare-synth-' + state._compareMode);
