...
const RECENT_KEY = 'solaris-model-recent';
const FAVORITES_KEY = 'solaris-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('solaris-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('solaris-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('solaris:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('solaris:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `solaris-model-favorites[0]`
