...
const _ACTION_RECENTS_KEY = 'solaris-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'solaris-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Solaris';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Solaris' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
