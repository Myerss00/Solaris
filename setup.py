#!/usr/bin/env python3
"""Solaris — first-time setup script.
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("SOLARIS_ADMIN_USER", "").strip().lower()
        password = os.getenv("SOLARIS_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] SOLARIS_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] SOLARIS_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not os.getenv("SOLARIS_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("SOLARIS_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("SOLARIS_ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set SOLARIS_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def main():
    ...
    if not os.getenv("SOLARIS_SKIP_RUN_HINT"):
