                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Solaris (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Solaris" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "solaris_kind" if "solaris_kind" in cols else "'scheduled' AS solaris_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                solaris_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Solaris-Origin"] = "solaris-ui"
                outer["X-Solaris-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", solaris_kind or "scheduled")[:64]
                outer["X-Solaris-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `solaris-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `SOLARIS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `solaris-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("SOLARIS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `SOLARIS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (SOLARIS_INPROCESS_POLLERS=0); "
            "drive `solaris-mail poll-scheduled` externally."
        )
        return
    import asyncio
