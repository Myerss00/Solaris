...
def _default_document_owner() -> str | None:
    ...
    """
    owner = os.environ.get("SOLARIS_DOCUMENT_OWNER", "").strip()
    if owner:
        return owner
    try:
    ...
    try:
        ...
        conn.execute("""
            ...
            (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr,
             attachments, send_at, created_at, status, account_id, solaris_kind, owner)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'agent_draft', ?, ?, ?)
        """, (
            pending_id,
...
def _create_email_draft_document(
    *,
    ...
    """Create an Solaris email compose document for user review. Does not send."""
...
def _draft_reply_to_email(uid, body, folder="INBOX", reply_all=False, account=None, title=None):
    """Create a threaded Solaris reply draft document. Does not send."""
    conn = _imap_connect(account)
    conn.select(_q(folder), readonly=True)
    status, msg_data = conn.uid("FETCH", _b(uid), "(BODY.PEEK[])")
    conn.logout()
...
async def _ai_draft_reply_to_email(uid, folder="INBOX", reply_all=False, account=None, title=None):
    """Generate a reply with Solaris' AI-reply prompt/style, then create a compose doc."""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description=(
                "List the email accounts configured in Solaris. Returns each account's "
        ...
        Tool(
            ...
            description=(
                ...
                "draft_email so the user can review and send from Solaris. "
        ...
        Tool(
            name="draft_email",
            description=(
                "Create a new Solaris email compose draft document. This DOES NOT send. "
                ...
                "can edit or press Send in Solaris. "
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "cc": {"type": "string", "description": "CC address(es), comma-separated (optional)"},
                    "bcc": {"type": "string", "description": "BCC address(es), comma-separated (optional)"},
                    "title": {"type": "string", "description": "Optional Solaris document title"},
                    **ACCOUNT_PROP,
                },
                "required": ["to", "subject", "body"],
            },
        ...
        Tool(
            ...
            description=(
                ...
                "review and send from Solaris. Automatically threads the reply with "
        ...
        Tool(
            ...
            description=(
                "Create an Solaris email reply draft document for an existing email UID. "
        ...
        Tool(
            ...
            description=(
                "Generate an AI reply using Solaris' existing AI Reply behavior, "
    ...
    try:
        ...
        if name == "list_emails":
            ...
            if result.get("pending"):
                return [TextContent(
                    ...
                    text=(
                        ...
                        "Nothing has been sent yet. Review and approve it in Solaris before delivery."
                    ),
                )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created Solaris email draft `{result['title']}` "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in Solaris to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Created Solaris reply draft `{result['title']}` for UID {uid} "
                    f"(document ID: {result['doc_id']}){acct_note}. "
                    "It has not been sent; open the document in Solaris to review and send."
                ),
            )]
            ...
            return [TextContent(
                type="text",
                text=(
                    f"Generated AI reply and created Solaris compose draft "
