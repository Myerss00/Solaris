...
class TestRunningInContainer:
    """Detect whether the Solaris process itself runs inside a container."""
...
class TestPackageProbeStatus:
    ...
    def test_vllm_cli_without_dist_is_external_for_update(self):
        ...
        assert "outside Solaris" in status.note
    ...
    def test_apfel_does_not_use_generic_outside_solaris_note(self):
        status = _package_pip_update_status(
        ...
        assert status.available is False
        assert "Update this system dependency outside Solaris." not in status.note
