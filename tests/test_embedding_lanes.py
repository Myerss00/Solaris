...
def test_build_embedding_lanes_keeps_custom_and_fastembed_dimensions_separate(monkeypatch):
    ...
    built = build_embedding_lanes("solaris_memories")

    assert [lane.name for lane in built] == [LANE_CUSTOM, LANE_FASTEMBED]
    assert built[0].collection_name == "solaris_memories_custom"
    assert built[0].dimension == 768
    assert built[1].collection_name == "solaris_memories_fastembed"
...
def test_build_embedding_lanes_recreates_only_custom_when_fingerprint_changes(monkeypatch):
    ...
    built = build_embedding_lanes("solaris_rag")

    assert "solaris_rag_custom" in fake.deleted
    assert fake.collections["solaris_rag_custom"].count() == 1
    assert len(fake.collections["solaris_rag_custom"].rows["old"]["embedding"]) == 1024
    assert fake.collections["solaris_rag_fastembed"].count() == 1
...
def test_lane_reset_reembeds_existing_documents_on_fingerprint_change(monkeypatch):
    ...
    assert "solaris_memories_custom" in fake.deleted
    rebuilt = fake.collections["solaris_memories_custom"]
...
def test_lane_reset_keeps_existing_collection_when_reembed_fails(monkeypatch):
    ...
    assert "solaris_memories_custom" not in fake.deleted
    assert fake.collections["solaris_memories_custom"].count() == 1
    assert len(fake.collections["solaris_memories_custom"].rows["existing-memory"]["embedding"]) == 384
...
def test_lane_reset_keeps_existing_collection_when_preserve_read_fails(monkeypatch):
    ...
    assert built == []
    assert "solaris_memories_custom" not in fake.deleted
    assert "solaris_memories_custom" in fake.collections
...
def test_lane_reset_restores_existing_collection_when_rewrite_fails(monkeypatch):
    ...
    fake.fail_next_add_for["solaris_memories_custom"] = 1
    _patch_chroma(monkeypatch, fake)
    ...
    assert built == []
    restored = fake.collections["solaris_memories_custom"]
...
def test_build_embedding_lanes_uses_fastembed_when_custom_unavailable(monkeypatch):
    ...
    built = build_embedding_lanes("solaris_tool_index")

    assert [lane.name for lane in built] == [LANE_FASTEMBED]
    assert built[0].collection_name == "solaris_tool_index_fastembed"
...
def test_memory_vector_store_writes_both_lanes_and_prefers_custom(monkeypatch):
    ...
    assert fake.collections["solaris_memories_custom"].count() == 1
    assert fake.collections["solaris_memories_fastembed"].count() == 1
...
def test_memory_search_merges_fallback_only_results_before_limit():
    custom_collection = FakeCollection("solaris_memories_custom", metadata={"embedding_lane": "custom"})
    fast_collection = FakeCollection("solaris_memories_fastembed", metadata={"embedding_lane": "fastembed"})
    ...
    custom_lane = EmbeddingLane(
        ...
        collection_name="solaris_memories_custom",
        model="nomic",
    ...
    fast_lane = EmbeddingLane(
        ...
        collection_name="solaris_memories_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_vector_rag_writes_both_lanes_and_falls_back_to_fastembed(monkeypatch):
    ...
    assert "solaris_rag_custom" not in fake.collections
    assert fake.collections["solaris_rag_fastembed"].count() == 1
...
def test_vector_rag_batch_index_continues_when_custom_lane_fails(monkeypatch, tmp_path):
    ...
    assert fake.collections["solaris_rag_custom"].count() == 0
    assert fake.collections["solaris_rag_fastembed"].count() == 1
...
def test_vector_rag_batch_index_reports_failure_when_all_lanes_fail(monkeypatch, tmp_path):
    ...
    assert fake.collections["solaris_rag_custom"].count() == 0
    assert fake.collections["solaris_rag_fastembed"].count() == 0
...
def test_tool_index_indexes_and_retrieves_from_available_lanes(monkeypatch):
    ...
    assert fake.collections["solaris_tool_index_custom"].count() > 0
    assert fake.collections["solaris_tool_index_fastembed"].count() > 0
...
def test_tool_index_builtin_indexing_fails_when_all_lanes_fail():
    custom_lane = EmbeddingLane(
        ...
        collection=FakeCollection("solaris_tool_index_custom", metadata={"embedding_lane": "custom"}),
        collection_name="solaris_tool_index_custom",
        model="nomic",
        url="http://embeddings/v1",
        dimension=768,
        fingerprint="custom",
    ...
    fast_lane = EmbeddingLane(
        ...
        collection=FakeCollection("solaris_tool_index_fastembed", metadata={"embedding_lane": "fastembed"}),
        collection_name="solaris_tool_index_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_tool_index_retrieval_continues_when_custom_lane_query_fails():
    custom_collection = FakeCollection("solaris_tool_index_custom", metadata={"embedding_lane": "custom"})
    fast_collection = FakeCollection("solaris_tool_index_fastembed", metadata={"embedding_lane": "fastembed"})
    fast_collection.add(
        ids=["builtin_bash"],
        embeddings=[[0.0] * 384],
        documents=["Tool: bash\nRun shell commands"],
...
def test_legacy_collection_backfills_fastembed_lane(monkeypatch):
    fake = FakeChroma()
    legacy = fake.get_or_create_collection("solaris_memories", metadata={"hnsw:space": "cosine"})
    legacy.add(
        ids=["legacy-memory"],
        embeddings=[[0.0] * 384],
    ...
    assert fake.collections["solaris_memories"].count() == 1
    assert fake.collections["solaris_memories_fastembed"].count() == 1
...
def test_legacy_collection_backfills_custom_only_lane(monkeypatch):
    ...
    assert store.count() == 1
    assert "solaris_memories_fastembed" not in fake.collections
    assert fake.collections["solaris_memories_custom"].count() == 1
    assert len(fake.collections["solaris_memories_custom"].rows["legacy-memory"]["embedding"]) == 768
...
def test_legacy_migration_continues_when_custom_backfill_fails(monkeypatch):
    ...
    assert fake.collections["solaris_memories_custom"].count() == 0
    assert fake.collections["solaris_memories_fastembed"].count() == 1
...
def test_legacy_migration_resumes_partial_lane_backfill(monkeypatch):
    ...
    partial = fake.get_or_create_collection("solaris_memories_fastembed", metadata={"embedding_lane": "fastembed"})
    partial.add(
        ids=["legacy-1"],
        embeddings=[[0.0] * 384],
    ...
    assert set(fake.collections["solaris_memories_fastembed"].get()["ids"]) == {"legacy-1", "legacy-2"}
...
def test_memory_rebuild_does_not_reimport_legacy_collection(monkeypatch):
    ...
    inactive_custom = fake.get_or_create_collection("solaris_memories_custom", metadata={"embedding_lane": "custom"})
    inactive_custom.add(
        ids=["stale-custom"],
        embeddings=[[0.0] * 768],
    ...
    assert "solaris_memories" not in fake.collections
    assert "solaris_memories_custom" not in fake.collections
    assert fake.collections["solaris_memories_fastembed"].count() == 1
    assert fake.collections["solaris_memories_fastembed"].get()["ids"] == ["current-memory"]


def test_memory_remove_deletes_inactive_lane_collection(monkeypatch):
    fake = FakeChroma()
    custom_collection = fake.get_or_create_collection("solaris_memories_custom", metadata={"embedding_lane": "custom"})
    fast_collection = fake.get_or_create_collection("solaris_memories_fastembed", metadata={"embedding_lane": "fastembed"})
    custom_collection.add(
        ids=["mem-1"],
        embeddings=[[0.0] * 768],
...
def test_rag_rebuild_does_not_reimport_legacy_collection(monkeypatch, tmp_path):
    fake = FakeChroma()
    legacy = fake.get_or_create_collection("solaris_rag", metadata={"hnsw:space": "cosine"})
    legacy.add(
        ids=["stale-doc"],
        embeddings=[[0.0] * 384],
    ...
    inactive_custom = fake.get_or_create_collection("solaris_rag_custom", metadata={"embedding_lane": "custom"})
    inactive_custom.add(
    ...
    assert "solaris_rag" not in fake.collections
    assert "solaris_rag_custom" not in fake.collections
    assert fake.collections["solaris_rag_fastembed"].count() == 0
...
def test_rag_remove_directory_deletes_inactive_lane_collection(monkeypatch, tmp_path):
    fake = FakeChroma()
    legacy_collection = fake.get_or_create_collection("solaris_rag", metadata={"hnsw:space": "cosine"})
    custom_collection = fake.get_or_create_collection("solaris_rag_custom", metadata={"embedding_lane": "custom"})
    fast_collection = fake.get_or_create_collection("solaris_rag_fastembed", metadata={"embedding_lane": "fastembed"})
    ...
    fast_lane = EmbeddingLane(
        ...
        client=FakeEmbedder(384, "mini", "local://fastembed"),
        collection=fast_collection,
        collection_name="solaris_rag_fastembed",
        model="mini",
        url="local://fastembed",
        dimension=384,
        fingerprint="fast",
...
def test_vector_rag_uses_keyword_fallback_when_all_lanes_query_fail():
    collection = FakeCollection("solaris_rag_fastembed", metadata={"embedding_lane": "fastembed"})
    collection.add(
        ids=["doc-1"],
        embeddings=[[0.0] * 384],
