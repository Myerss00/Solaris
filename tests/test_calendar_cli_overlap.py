"""Regression: `solaris-calendar list` must select events that OVERLAP the
...
`solaris-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "solaris-calendar"
    loader = importlib.machinery.SourceFileLoader("solaris_calendar_cli", str(path))
