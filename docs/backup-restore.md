...
Solaris keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/solaris-backup` tool
...
```bash
# Create a snapshot → backups/solaris-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/solaris-backup snapshot

# List existing snapshots (most recent first)
./scripts/solaris-backup list

# Check a tarball's integrity without extracting it
./scripts/solaris-backup verify backups/solaris-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/solaris-backup restore backups/solaris-backup-20260101-120000.tar.gz --yes
```
...
./scripts/solaris-backup snapshot --out /mnt/nas/solaris-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/solaris-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/solaris && ./scripts/solaris-backup snapshot --out "/mnt/nas/solaris-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `solaris-backup` therefore does not capture the Docker
