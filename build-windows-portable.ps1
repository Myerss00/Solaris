...
<#
  Build a portable Windows distribution for Solaris.

  Output layout:
    dist\Solaris\Solaris.exe
    dist\Solaris\static\...
    dist\Solaris\scripts\...
    dist\Solaris\mcp_servers\...
    dist\Solaris\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Solaris @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Solaris" -ForegroundColor Green
